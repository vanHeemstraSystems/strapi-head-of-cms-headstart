...
Build and tag the Docker image, and run the container specifying its name as "pwc-cms-dev" to distinguish it from possible other stacks that are called "app" (the default name, based on the root directory):

```
$ cd containers/app
$ docker-compose --file docker-compose.dev.yml --project-name pwc-cmsgui-dev up --build -d
```
